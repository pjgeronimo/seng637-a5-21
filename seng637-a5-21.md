...
The results of the RDC are not a good representation of the system. An assumption was made that the failures are evenly distributed over their time interval, which is likely not the case, and the RDC itself is only limited to 16 data points as its input, which is less than 20% of the entire dataset.
