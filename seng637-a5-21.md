...
In order to find the minimum MTTF for the SUT to be considered acceptable, no points in the graph should be in the red "Reject" region. Using trial and error on the "Failure Intensity Objective" parameters, the **minimum FIO to achieve an acceptable SUT is 5 failures per time interval**. However, it is important to note that these results may not be the best indication for the reliability of the system, due to the RDC's limitation of only 16 data points. 
